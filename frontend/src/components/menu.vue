        ...
        <div class="menu_top">
            <div class="menu_box" :class="{ 'has-submenu': item.children }" v-for="(item, index) in topMenuItems" :key="index">
                ...
                <div v-if="batchMode && item.path === 'creatChat' && !uiStore.sidebarCollapsed" class="batch-inline-footer">
                    ...
                    <t-button size="small" theme="danger" variant="base" :disabled="batchSelectedIds.length === 0" :loading="batchDeleting" @click="handleInlineBatchDelete">
...
<script setup lang="ts">
    ...
    // 通过参数去隐藏一些东西
    const hideUser = computed(() => {
        return route.query?.hide != null && (route.query?.hide === "true" || route.query?.hide === "1")
    })
    console.log("读取参数2", hideUser, route);
    // 分离上下两部分菜单
